...
class MIMOWhoGeneralDotProductAttention(nn.Module):
    ...
    def forward(self, qu, k, v, sparse=True):
        ...
        for bi in range(attn_orig.shape[0]):
            up = torch.triu(attn_orig[bi],diagonal=1,out=None)[:-1,]   # 只有上三角矩阵不是0
            dow = torch.tril(attn_orig[bi],diagonal=-1,out=None)[1:,]   # 只有下三角矩阵不是0
            del_diag_att_orig_per_sample = torch.unsqueeze((up+dow),dim=0)
            del_diag_att_orig.append(del_diag_att_orig_per_sample) # delete diagonal
...
class LearnWho2Com(nn.Module):
    ...
    def forward(self, inputs, training=True, inference='argmax_test'):
